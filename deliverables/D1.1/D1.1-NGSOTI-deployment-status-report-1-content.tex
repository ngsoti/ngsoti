...
The alignment with the objectives in the grant agreements are shown in the table \ref{obj}.
\begin{table}[h]
\centering
\caption{Objective Description Table}
\label{tab:objective-description}
\begin{tabular}{|p{1cm}|p{0.2\linewidth}|p{0.2\linewidth}|p{0.2\linewidth}|p{0.2\linewidth}|p{1cm}}\hline
Number & Objective description & State & Achievements & References in the report \\
\hline
WP2.O1 & A first objective is to setup the technical infrastructure & ONGOING & The NGSOTI (Next Generation Security Operations Training Infrastructure) project is a virtual training infrastructure designed for security operations training. It consists of three physical servers: ngsoti1, ngsoti2, and ngsoti3, which host LXC containers, datasets, and auxiliary GPU-accelerated workloads, respectively. The virtual training environment is built around LXC and Tailscale technologies, allowing for the creation of containers and a virtual network that users can access from anywhere. The infrastructure includes components such as headscale for authentication and Access Control Lists, and documentation is available in the project's internal notes. Restena has made progress on the project, including procuring hardware, setting up equipment, and initiating Blackhole data processing, as well as setting up a phishing platform for training activities. & Chapter 2 and 3 \\
\hline
\end{tabular}
