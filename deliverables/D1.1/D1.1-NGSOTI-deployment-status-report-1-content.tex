...
The NGSOTI (Next Generation Security Operations Training Infrastructure) project is a virtual training infrastructure designed for security operations training. It consists of three physical servers: ngsoti1, ngsoti2, and ngsoti3, which host LXC containers, datasets, and auxiliary GPU-accelerated workloads, respectively. The virtual training environment is built around LXC and Tailscale technologies, allowing for the creation of containers and a virtual network that users can access from anywhere. The infrastructure includes components such as headscale for authentication and Access Control Lists, and documentation is available in the project's internal notes. Restena has made progress on the project, including procuring hardware, setting up equipment, and initiating Blackhole data processing, as well as setting up a phishing platform for training activities. &
2,3 \\
\hline
\end{tabular}
